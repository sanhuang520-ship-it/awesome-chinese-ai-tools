...
def build_skills_md():
    ...
    L.append("|---|---|---|---|")
    L.append(f"| 审查中文网页排版 | `npx skills add https://github.com/{REPO} --skill chinese-typography` | `请检查这段 CSS 的中文字体、行高、断词和两端对齐问题，只审查，不修改文件。` | [Codex 单任务实测](cases/chinese-typography-codex.md) |")
    ...
    L.append("**[→ 查看全部 13 个单项安装命令与可复制首次任务](https://sanhuang520-ship-it.github.io/awesome-chinese-ai-tools/try-agent-skills/)**：7 条历史逐字原文与 6 条前瞻任务严格分开；前瞻任务中 5 条已执行、1 条仍待测。\n")
