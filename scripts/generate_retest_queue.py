...
def load_items():
    ...
    items = data["items"]
    if len(items) != 6 or any(item.get("status") != "planned" for item in items):
        raise ValueError("retest queue must contain exactly six planned items")
    return items
