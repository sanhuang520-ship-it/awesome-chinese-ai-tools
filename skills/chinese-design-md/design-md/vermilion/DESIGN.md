---
version: 1.0
name: Vermilion-chinese-design
description: "正红偏橙的朱砂，配暖白底。喜庆但不俗，适合节日、活动页、需要「有温度」的转化页面。 Orange-leaning cinnabar red on warm white. Festive without being cheap."
...
正红偏橙的朱砂，配暖白底。喜庆但不俗，适合节日、活动页、需要「有温度」的转化页面。

**出处**：朱砂：印章与传统年画的主色，正红偏橙。
...
| Token | 值 | 用途 |
|---|---|---|
| `primary` | `#B23A2E` | 主行动点、链接、关键强调。**一屏不超过 2–3 处** |
| `primary-weak` | `#E0B0A8` | 强调色的浅版，用于背景高亮、选中态、边框 |
...
|---|---|---|
| `canvas` | `#FBF7F4` | 页面底色 |
| `surface` | `#F3E9E3` | 卡片、引用块、代码块底 |
| `hairline` | `#E0CCC3` | 分割线、边框。**只用 1px** |

### Text

| Token | 值 | 用途 |
|---|---|---|
| `ink-strong` | `#1A1412` | 标题 |
| `ink` | `#38302C` | 正文 |
| `ink-soft` | `#6E5F58` | 辅助文字、说明、时间戳 |
...
input:focus {
  border-color: #B23A2E;
  outline: 2px solid #E0B0A8;
}
```

⚠️ **placeholder 用 `ink-soft`（#6E5F58）**，不要更浅——中文笔画密，太浅会糊。

## Do's and Don'ts

### Do

- 朱砂只用于主行动点和关键强调，用多了会腻
- 大面积留白衬托，让红色有分量
- 适合做节日主题，但注意节庆禁忌（清明、中元不用大红）
- 行高保持 1.75，这是中文可读性的底线
- 强调用着重号，不用斜体

### Don't

- 别配金色做成「土豪风」，这套是雅红不是俗红
- 别整屏铺红底
- 别用于哀悼、纪念类内容
