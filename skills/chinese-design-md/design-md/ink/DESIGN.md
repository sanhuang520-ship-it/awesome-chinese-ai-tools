---
version: 1.0
name: Ink Wash-chinese-design
description: "宣纸的米白、墨的层次、大量留白。最克制的一套，适合长文阅读、文化类内容、需要「安静」的界面。 Rice-paper off-white, graded ink tones, generous emptiness. The most restrained set — long-form reading, cultural content, interfaces that need to be quiet."
...
宣纸的米白、墨的层次、大量留白。最克制的一套，适合长文阅读、文化类内容、需要「安静」的界面。

**出处**：传统水墨画的墨分五色：焦、浓、重、淡、清。
...
| Token | 值 | 用途 |
|---|---|---|
| `primary` | `#2E2A26` | 主行动点、链接、关键强调。**一屏不超过 2–3 处** |
| `primary-weak` | `#C4BCAF` | 强调色的浅版，用于背景高亮、选中态、边框 |
...
|---|---|---|
| `canvas` | `#F7F5F0` | 页面底色 |
| `surface` | `#EFEBE3` | 卡片、引用块、代码块底 |
| `hairline` | `#DDD7CC` | 分割线、边框。**只用 1px** |

### Text

| Token | 值 | 用途 |
|---|---|---|
| `ink-strong` | `#1C1A17` | 标题 |
| `ink` | `#3A3733` | 正文 |
| `ink-soft` | `#6E6860` | 辅助文字、说明、时间戳 |
...
input:focus {
  border-color: #2E2A26;
  outline: 2px solid #C4BCAF;
}
```

⚠️ **placeholder 用 `ink-soft`（#6E6860）**，不要更浅——中文笔画密，太浅会糊。
...
- 用留白划分层次，不要靠边框和分割线堆
- 强调色只用在真正要强调的地方，一屏不超过两处
- 标题用衬线（宋体），正文可用无衬线，形成质感对比
