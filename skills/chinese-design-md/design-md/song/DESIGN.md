---
version: 1.0
name: Song Dynasty-chinese-design
description: "素雅、克制、文人气。青瓷般的灰绿，几乎没有装饰。适合茶饮、书店、高端服务、任何想显得「有文化但不张扬」的地方。 Understated, restrained, scholarly. Celadon grey-green with almost no ornament."
...
素雅、克制、文人气。青瓷般的灰绿，几乎没有装饰。适合茶饮、书店、高端服务、任何想显得「有文化但不张扬」的地方。

**出处**：宋代美学：极简、单色釉、器型即装饰。
...
| Token | 值 | 用途 |
|---|---|---|
| `primary` | `#6E9C93` | 主行动点、链接、关键强调。**一屏不超过 2–3 处** |
| `primary-weak` | `#BCD2CD` | 强调色的浅版，用于背景高亮、选中态、边框 |
...
|---|---|---|
| `canvas` | `#F4F6F5` | 页面底色 |
| `surface` | `#E8EDEC` | 卡片、引用块、代码块底 |
| `hairline` | `#D2DCDA` | 分割线、边框。**只用 1px** |

### Text

| Token | 值 | 用途 |
|---|---|---|
| `ink-strong` | `#1B2422` | 标题 |
| `ink` | `#37423F` | 正文 |
| `ink-soft` | `#6B7875` | 辅助文字、说明、时间戳 |
...
| 正文 / 底色 | **9.61:1** | ✅ |
| 标题 / 底色 | **14.63:1** | ✅ |
| 辅助文字 / 底色 | 4.23:1 | ⚠️ 未达标 |
| 主色 / 底色 | 2.83:1 | ⚠️ 未达标 |

> ⚠️ **辅助文字色未达 AA（4.23:1）**。只用于非关键信息（时间戳、脚注）；
> 需要正文级可读性时改用 `ink`（#37423F）。

> ⚠️ **主色作为文字色未达 AA（2.83:1）**。这套的主色适合做
...
input:focus {
  border-color: #6E9C93;
  outline: 2px solid #BCD2CD;
}
```

⚠️ **placeholder 用 `ink-soft`（#6B7875）**，不要更浅——中文笔画密，太浅会糊。
