...
安装与自动触发不是一回事。本站 13 个原创 Skill 已完成 CLI 发现与当前 Codex 安装内容核验，`chinese-typography` 已完成首个[不点名 Skill 的自动触发实测](cases/chinese-typography-codex.md)；其余逐项触发、Claude Code 和 Cursor 仍在继续测试，详见 **[兼容性实测表](COMPATIBILITY.md)**。
