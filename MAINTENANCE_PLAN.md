...
- 每周一自动复检 Skill 来源仓库与官方信源（也可手动触发）。AI 工具链接已于 2026-09-14 归档，不再复检。
...
- [x] 在固定查询修改前快照中增加 `chinese ai skills`，并记录仓库 description、homepage 与 topics，避免后续比较丢失 Profile 上下文。
- [ ] 经可验证的设置入口同步准确的中英 repository description 和常用 topic 变体；同步前后只观察排名，不做单因果归因。
- [x] 将 GitHub description、homepage 与 topics 固化为可审查配置，并提供只读公开 API 漂移检查器。

### 3. 搜索与内容入口

- [x] 在默认 README 首屏与首页生成元数据中加入准确的 `Chinese Agent Skills / Chinese AI Skills` 双语定位，并发布包含实际安装、证据、安全与贡献路径的英文落地页，不制作空关键词页。
...
- [x] 从 `data/skills.json` 生成无需 JavaScript 的完整静态目录页，按场景公开全部条目、来源身份与证据边界，并用离线检查阻止页面和数据漂移。
