...
2026-09-03 起本仓库开始独立实测第三方 Skill 并公布结果——检索确认这件事此前没人在做
（`claude skills benchmark trigger test` 返回 0 个结果；现有工具都是给作者测自己用的）。

规矩与流程见 **[测别人的 Skill：流程与规矩](THIRD-PARTY-TESTING.md)**，
其中最要紧的一条是**只用它自己声明的契约当标准**——对声明「不要自动触发」的 Skill，
「没触发」才算通过。首例见
[`chinese-git-workflow`](cases/third-party-chinese-git-workflow-2026-09-03.md)。
